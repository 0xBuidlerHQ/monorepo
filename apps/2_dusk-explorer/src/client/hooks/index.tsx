...
const shortcuts: { name: string; shortcut: string }[] = [{ name: "Todo", shortcut: "Todo" }];
			...
			cell: ({ row: { original: block } }) => (
				<Button asChild variant="ghost" className="h-auto p-0 hover:bg-transparent">
					<Link href={BLOCK.href + block.id}>
						<Box>
							<SP className="font-semibold underline underline-offset-[3px]">{block.id}</SP>
		...
		},
		{
			...
			cell: ({ row: { original: block } }) => (
				<div className="flex flex-col justify-center">
					...
					<div className="text-[13px]">
						<span className="text-xs text-muted-foreground">Total:</span> {block.txsNb.toString()}
					</div>
				</div>
			),
		},
...
export { useLatestBlocks, useLatestTxs, shorten0x, useBanner, useChart, useClient, type Client };
